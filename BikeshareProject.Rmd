...
#### Create a new dataset of the weekday, month and city for each start time. Weekday Levels: Sun < Mon < Tue < Wed < Thu < Fri < Sat
    month = c(month(chi$Start.Time), month(ny$Start.Time), month(wash$Start.Time))
    weekday = c(wday(chi$Start.Time), wday(ny$Start.Time), wday(wash$Start.Time))
    city = c(rep("Chicago", times = dim(chi)[1]), rep("New York", times = dim(ny)[1]), 
             rep("Washington", times = dim(wash)[1]))
    all.start = data.frame(month, weekday, city)

#### Plot the frequency of each weekday
    wday_plot = ggplot(aes(x=weekday), data = all.start) +
      geom_histogram(stat = 'count', color = 'black', fill = 'blue') +
      scale_x_continuous(breaks = c(1:7), labels = c('Sun','Mon','Tue','Wed','Thu','Fri','Sat'))
    
    wday_plot + ggtitle('Total Bike Rentals in Chicago, Washington and New York') +
      scale_y_continuous(breaks = c(seq(0,150000, 20000)))

#### To get summary statistics, create a frequency table from the weekday data
    tbl_wday = table(all.start$weekday)
    tbl_wday
    
#### Get the maximum weekday with max()
    max(tbl_wday)
    
#### Results: The chart shows that Wednesdays have had the most bike rentals in all 3 cities combined. The summary statistics show that the highest count of bike rentals is 142,773, which is the count for Wednesdays in the chart. 
...
#### Plot the frequency of each weekday per month
    ...
    ggtitle('Bike Rentals Per Weekday Per Month in Chicago, Washington and New York')
    
#### To get summary statistics, create a frequency table from the weekday and month data.  

    tbl_mo.wday = table(all.start$month, all.start$weekday)
    tbl_mo.wday
...
#### Plot the weekday per city       
      ...
      ggtitle('Bike Rentals Per Weekday Per City')
    
#### To get summary statistics, create a frequency table from the weekday and city data.
    tbl_cty.wday = table(all.start$city, all.start$weekday)
    tbl_cty.wday        
